...
sub get_time {
 ...
 $getdate=$gettime->prepare("select now(),substring(now() - interval 15 minute,1,4),substring(now() - interval 15 minute,6,2),substring(now() - interval 15 minute,9,2),substring(now() - interval 15 minute,12,2),substring(now() - interval 15 minute,15,2)");
 $getdate->execute();
